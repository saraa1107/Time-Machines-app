...
export default function ClsVScreen({ navigation }) {
  ...
  const [nivel2PressCount, setNivel2PressCount] = useState(0);

  const handleNivel1Press = () => {
    if (nivel1PressCount === 0) {
      setNivel1PressCount(1);
    } else {
      navigation.navigate('PaleoliticScreen');
    }
  };

  const handleNivel2Press = () => {
    if (nivel2PressCount === 0) {
      setNivel2PressCount(1);
    } else {
      navigation.navigate('NeoliticScreen');
    }
  };
  ...
  return (
    ...
    >
      <View style={styles.container}>
        <Text style={styles.title}>Învățăceii clasei a V-a</Text>

        <TouchableOpacity style={styles.button} onPress={handleNivel1Press}>
          <Text style={styles.buttonText}>Nivel 1</Text>
          {nivel1PressCount === 1 && (
            <Text style={styles.infoText}>Paleolitic</Text>
          )}
        </TouchableOpacity>

        <TouchableOpacity style={styles.button} onPress={handleNivel2Press}>
          <Text style={styles.buttonText}>Nivel 2</Text>
          {nivel2PressCount === 1 && (
            <Text style={styles.infoText}>Neolitic</Text>
          )}
        </TouchableOpacity>
